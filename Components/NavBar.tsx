...
const NavBar = () => {
  return (
     <div className="flex justify-between pt-1.5 bg-transparent absolute z-50 gap- pb-30">
      ...
      <div className="flex px-5 items-center gap-4 md:gap-8 mr-5  border rounded-4xl p-1.5">
        {NavLinks.map((item, index) => (
          <Link className="hover:bg-gray-400 hover:rounded  transition" key={index} href={item.path}>
            {item.name}
          </Link>
        ))}
      </div>
